...
import codepadRaw from "../codepad-server/src/codepad.rs?raw";
import languages from "./languages.json";
import animals from "./animals.json";
import Codepad, { UserInfo } from "./codepad";
...
function App() {
  ...
  useEffect(() => {
    if (editor?.getModel()) {
      ...
      return () => {
        codepad.current?.dispose();
        codepad.current = undefined;
      };
    }
  ...
  useEffect(() => {
    if (connection === "connected") {
      codepad.current?.setInfo({ name, hue });
    }
  }, [connection, name, hue]);

  function handleChangeLanguage(language: string) {
    setLanguage(language);
    if (codepad.current?.setLanguage(language)) {
      toast({
      ...
      <Flex flex="1 0" minH={0}>
        ...
        >
          ...
          <Text fontSize="sm" mb={1.5}>
            <strong>Codepad</strong> is an open-source collaborative text editor
          ...
          <Text fontSize="sm" mb={1.5}>
            ...
            <Link
              color="blue.600"
              fontWeight="semibold"
              href="https://github.com/Sumitumar-17/codepad"
              isExternal
            >
              GitHub repository
            </Link>{" "}
