...
use crate::{database::Database, codepad::Codepad};
...
impl Document {
    fn new(codepad: Arc<Codepad>) -> Self {
        Self {
...
async fn socket_handler(id: String, ws: Ws, state: ServerState) -> Result<impl Reply, Rejection> {
    ...
    let mut entry = match state.documents.entry(id.clone()) {
        ...
        Entry::Vacant(e) => {
            let codepad = Arc::new(match &state.database {
                Some(db) => db.load(&id).await.map(Codepad::from).unwrap_or_default(),
                None => Codepad::default(),
            });
            if let Some(db) = &state.database {
                tokio::spawn(persister(id, Arc::clone(&codepad), db.clone()));
            }
            e.insert(Document::new(codepad))
        }
    };

    let value = entry.value_mut();
    value.last_accessed = Instant::now();
    let codepad = Arc::clone(&value.codepad);
    Ok(ws.on_upgrade(|socket| async move { codepad.on_connection(socket).await }))
}
...
async fn persister(id: String, codepad: Arc<Codepad>, db: Database) {
    let mut last_revision = 0;
    while !codepad.killed() {
        let interval = PERSIST_INTERVAL
            + rand::thread_rng().gen_range(Duration::ZERO..=PERSIST_INTERVAL_JITTER);
        time::sleep(interval).await;
        let revision = codepad.revision();
        if revision > last_revision {
            info!("persisting revision {} for id = {}", revision, id);
            if let Err(e) = db.store(&id, &codepad.snapshot()).await {
