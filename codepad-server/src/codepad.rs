//! Eventually consistent server-side logic for Codepad.
...
impl From<PersistedDocument> for Codepad {
    fn from(document: PersistedDocument) -> Self {
        let mut operation = OperationSeq::default();
        operation.insert(&document.text);

        let codepad = Self::default();
        {
            let mut state = codepad.state.write();
...
impl Codepad {
    ...
    /// Returns if this Codepad object has been killed.
